#ifndef PAGERANK_UTILS_C
#define PAGERANK_UTILS_C

#include "pagerank_utils.h"

int set_core(pthread_t *thread, int tid, const int ncores) {
    // Set thread affinity
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
inline int my_pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine)(void *), void *arg, int tid, const int ncores) {
    const int rc = pthread_create(thread, attr, start_routine, arg);
    if (rc == 0) {
        set_core(thread, tid, ncores);
    }else{
...
void minHeapify(double v[], unsigned arr[], unsigned n, unsigned i) {
...
void kLargest(double v[], unsigned arr[], unsigned n, unsigned k) {
    assert(k<=n);
    assert(k>0);
